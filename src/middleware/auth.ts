...
// collection of its own — role/clientId/siteIds/permissions/status always comes from TLM's
// GET /users/me (the same endpoint TLM's own profile page uses), so a role change made in TLM
// propagates here within this bound instead of requiring a fresh login. Never persisted, process-memory only.
 ...
 * proves *who* cheaply, with no network call. Live role/clientId/siteIds/status then comes from
 * TLM's GET /users/me, cached briefly (see resolveProfile above) rather than fetched on every request.
 */
