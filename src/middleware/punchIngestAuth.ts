 ...
 * principal with no clientId/siteIds/permissions (every /punches write must specify its own
 * clientId in the body, validated per-request, since this key isn't scoped to one client in v1).
 * Otherwise falls through to the normal TLM-JWT `authenticate` — so a human user can also submit
 * punches directly if their own permissions allow it, without needing a separate credential.
 */
export function authenticatePunchIngestOrUser(req: Request, res: Response, next: NextFunction): void {
  ...
  if (ingestKey !== undefined) {
    ...
    req.auth = { userId: "punch-ingest-service", role: "PUNCH_INGEST", clientId: null, siteIds: [], permissions: [] };
    next();
    return;
  }
